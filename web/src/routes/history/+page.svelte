<script lang="ts">
	...
	function getRelativeTime(dateStr: string): string {
		try {
			const now = Date.now();
			const elapsedSec = Math.floor((now - new Date(dateStr).getTime()) / 1000);
	...
	onMount(() => {
		...
		const unsubReason = wsClient.onReasonUpdate((msg) => {
			...
			toggles = toggles.map((t) => (t.id === targetId ? { ...t, reason: msg.reason } : t));
		});
...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6">
	...
	{:else if error && toggles.length === 0}
		<div class="rounded-xl border border-red-500/20 bg-red-500/10 p-6 text-center text-red-300">
	...
	{:else if toggles.length === 0}
		<div class="rounded-xl border border-slate-800 bg-slate-900/40 p-12 text-center">
			...
			<h3 class="text-base font-semibold text-slate-200">No toggle history found</h3>
			<p class="mt-1 text-sm text-slate-400">Be the first to flip the switch!</p>
			<a
				href="/"
	...
	{:else}
				...
				>
					...
					<div class="flex flex-col gap-0.5 min-w-0 flex-1">
						<div class="flex items-center gap-2 flex-wrap">
							...
							<span class="text-[11px] text-slate-500">({getRelativeTime(item.created_at)})</span>
						</div>

						{#if item.reason}
							<p class="text-sm font-medium text-slate-200 flex items-start gap-1.5 mt-0.5 min-w-0">
								<svg
									xmlns="http://www.w3.org/2000/svg"
								...
								<span class="wrap-break-word min-w-0">"{item.reason}"</span>
							</p>
						{:else}
							<span class="text-xs italic text-slate-600">No reason specified</span>
						{/if}
					</div>
				</div>
			{/each}
		...
		<div bind:this={sentinelEl} class="mt-8 flex justify-center py-4 min-h-12.5">
			{#if loading}
			...
			{:else if !hasMore}
				<div class="text-xs text-slate-500 italic flex items-center gap-2">
